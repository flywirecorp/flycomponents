...
describe('PhoneInput', () => {
  ...
  it('handles on change event in input', () => {
    const countries = [
      { label: 'United States (+1)', value: 'us', dialingCode: '1', phonePattern: '+. (...) ...-....' }
    ]
    ...
    expect(
      onChange.calledWith(name, '+')
    ).to.be.true

    component.pressKey('1')

    expect(
      onChange.calledWith(name, '+1')
    ).to.be.true

    component.pressKey('2')

    expect(
      onChange.calledWith(name, '+1 (2')
    ).to.be.true
  })

  it('adds the dialingCode on click on a country', () => {
    const countries = [
      { label: 'Spain (+34)', value: 'es', dialingCode: '34', phonePattern: '+.. ... ... ...' },
      { label: 'United States (+1)', value: 'us', dialingCode: '1', phonePattern: '+. (...) ...-....' }
    ]
  ...
  describe('several countries with the same dialing code', () => {
    const countries = [
      { label: 'Canada (+1)', value: 'ca', dialingCode: '1', phonePattern: '+.. ... ... ...' },
      { label: 'United States (+1)', value: 'us', dialingCode: '1', phonePattern: '+.. ... ... ...' }
    ]
  ...
  describe('having read-only property', () => {
    ...
    it('passes read-only property to FlagSelector', () => {
      expect(
        component.flagSelector().prop('readOnly')
      ).to.be.true
    })
  })
})
